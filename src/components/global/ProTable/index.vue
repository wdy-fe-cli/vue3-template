  ...
  <div class="card table-main">
    ...
    <div class="table-header">
      ...
      <div v-if="toolButton" class="header-button-ri">
        <slot name="toolButton">
          <el-button v-blur icon="Refresh" circle @click="getTableList" />
          <!-- <el-button v-blur v-if="columns.length" icon="Printer" circle @click="print" /> -->
          <el-button v-blur v-if="columns.length" icon="Operation" circle @click="openColSetting" />
          <el-button v-blur v-if="searchColumns.length" icon="Search" circle @click="isShowSearch = !isShowSearch" />
        </slot>
      </div>
    </div>
    <!-- 表格主体 -->
    <el-table ref="tableRef" v-bind="$attrs" :data="data ?? tableData" :border="border" :row-key="rowKey" @selection-change="selectionChange">
      ...
      <template v-for="item in tableColumns" :key="item">
        ...
        <!-- other:添加纯文本居中形式（textCenter：重点表头居中） -->
        <TableColumn v-if="(!item.type || item.type === 'textCenter') && item.prop && item.isShow" :column="item">
...
const props = defineProps({
  ...
  searchCol: {
    ...
    type: Object,
    default: () => ({ xs: 2, sm: 3, md: 5, lg: 6, xl: 8 })
  }
})
...
const setEnumMap = async (col) => {
  ...
  const { content } = await col.enum()
  if (content.items && content.count) {
    enumMap.value.set(col.prop, content.items)
  } else {
    enumMap.value.set(col.prop, content)
  }
}
...
//     properties: flatColumns.value
//       ?.filter((item) => !['selection', 'index', 'expand'].includes(item.type) && item.isShow && item.prop !== 'operation')
//       .map((item) => ({ field: handleProp(item.prop), displayName: item.label })),
//     type: 'json',
//     gridHeaderStyle,
//     gridStyle
//   })
