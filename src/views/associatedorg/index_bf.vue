        ...
        <div class="main-box" style="max-height: 72vh" flex>
            ...
            <div style="flex: 1" flex="dir:top main:justify">
                <div class="goods-wrap ml12" style="flex-wrap: wrap" flex>
                    <div class="goods-item mb16 mr16" style="position: relative" v-for="(item, index) in goodsData"
                        :key="index" @click="onGoodsItem(item)">
                        <div flex style="flex-wrap: wrap">
                            ...
                            <div class="info ml8" flex="dir:top main:justify" style="width: 160px">
                                ...
                                <div class="color-warning ft12"
                                    v-else-if="item.isPrint === 1 && item.printerList && item.printerList.length > 0">
                                    {{ item.printerList.length > 1 ? `已设置 ${item.printerList.length} 个` :
                                        item.printerList[0].printerName }}
                                </div>
                                <div class="color-warning ft12" v-else>与分类一致</div>
                                <!-- <div flex="main:justify" v-if="item.isPrint !== 0"> -->
                                <div flex="main:justify">
                                    <el-button type="primary" link @click="setPrinter(item)"
                                        v-auth="'printerRelReceiptAdd'">设置打印机</el-button>
                ...
                <div flex="main:right">
                    <el-pagination background layout="total, prev, pager, next" :page-size="12" :total="goodsTotal"
