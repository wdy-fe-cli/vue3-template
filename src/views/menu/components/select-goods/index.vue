 ...
 */
const onConfirm = () => {
  emit('confirm', selectedGoodsIds.value, delGoodsIdList)
...
const show = (data) => {
  ...
  // if (data) {
  //   selectedGoodsIds.value = data.map((item) => item.id)
  // }

  // 初始请求参数
  let Params = { groupId: route.query.groupId, pageNumber: 1, pageSize: 9999 }
  // 获取全部商品
  getTableList(Params)
}

const getTableList = async (Params) => {
  let res = await http('findGoodsGroupItem', Params)
  // 每次更新数据，清空计算索引列表

  if (res.content.items && res.content.items.length > 0) {
    selectedGoodsIds.value = res.content.items.map((item) => item.goodsId)
  }
 ...
 * 查找商品列表
 */
const findPageListForGroup = async () => {
  const params = {
  ...
  // console.log(selectedGoodsIds)
  try {
    let res = await http('findPageListForGroup', params)
...
//     })
//   })
//   // spanArr.value = tagRowSpan(goodsData, 'id')
